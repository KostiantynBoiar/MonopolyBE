"""E2E: auction bidding after pass_buy."""
...
async def run_auction_flow() -> None:
    async with httpx.AsyncClient(timeout=10.0) as http:
        host_id, host_token = await _register(http, "auchost")
        guest_id, guest_token = await _register(http, "aucguest")
        ...
        async with _connect(session_id, host_token) as host_ws, _connect(
            ...
            for _ in range(MAX_ROLLS):
                ...
                if snap["turn"]["actions_available"]["can_buy"]:
                    await cur_ws.send(_envelope("game.pass_buy"))
                    auc_frame = await _recv_state(cur_ws)
                    await _recv_state(other_ws)
                    assert auc_frame["payload"]["auction"] is not None
                    auction_started = True

                    await cur_ws.send(_envelope("game.place_bid", {"amount": 60}))
                    bid_frame = await _recv_state(cur_ws)
                    guest_bid = await _recv_state(other_ws)
                    assert bid_frame["seq"] == guest_bid["seq"]
                    assert bid_frame["payload"]["auction"]["highest_bid"] == 60
                    break
            ...
            assert auction_started, "did not reach auction within roll limit"


@pytest.mark.e2e
def test_game_auction_e2e() -> None:
    try:
