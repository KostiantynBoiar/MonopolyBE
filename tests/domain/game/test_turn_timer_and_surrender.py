...
def _game(clock: FixedClock, n: int) -> GameState:
    ...
    return new_game(
        ...
        rng=SequencedRandom(),
        clock=clock,
        starting_balance=1500,  # type: ignore[arg-type]
    )
...
def test_three_strikes_auto_surrenders(clock: FixedClock) -> None:
    ...
    state = with_ownership(state, 2, a.id)  # Mediterranean Ave
    ...
    assert space_at(after.spaces, 2).owner_id is None  # property freed (buyable again)
...
def test_voluntary_surrender_frees_properties_and_splits_cash(clock: FixedClock) -> None:
    state = _game(clock, 3)
    a, b, c = state.players
    state = with_ownership(state, 4, a.id)  # Baltic Ave
    players = list(state.players)
    ...
    assert space_at(after.spaces, 4).owner_id is None
